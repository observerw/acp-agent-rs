...
/// Content type required by the HTTP/2 byte-stream transport.
...
mod tests {
    ...
    fn prepared_command(program: &str, args: &[&str]) -> PreparedCommand {
        PreparedCommand {
            spec: crate::runtime::prepare::CommandSpec {
        ...
        F: std::future::Future,
    {
        tokio::time::timeout(std::time::Duration::from_secs(2), future)
            .await
            .expect("timed out waiting for HTTP transport test operation")
    }
    ...
    async fn http_transport_rejects_second_stream() {
        ...
        let server = tokio::spawn(async move {
            let (socket, _) = listener
                ...
                .await
                .context("failed to accept HTTP client")?;
            serve_h2_connection(prepared_command("sh", &["-c", "cat"]), "demo-agent", socket).await
        });
